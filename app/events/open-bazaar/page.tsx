...
export const metadata: Metadata = {
  title: "Pilih Kategori Bazaar",
  description: "Pilih kategori Open Bazaar yang ingin Anda ikuti: Tingkat Mahasiswa atau UMUM.",
};

export default function BazaarCategoryPage() {
  const mahasiswaEvent = getEventBySlug("open-bazaar-mahasiswa");
  const umumEvent = getEventBySlug("open-bazaar-umum");

  if (!mahasiswaEvent || !umumEvent) {
    notFound();
  }

  const bazaarEvents = [mahasiswaEvent, umumEvent];
        ...
        <Container className="relative z-10 block w-full text-center mb-16">
          <div className="mx-auto max-w-3xl">
            ...
            <h1 className="font-heading text-4xl md:text-6xl font-bold text-white mb-6 text-glow">
              Pilih Kategori Bazaar
            </h1>
            <p className="text-lg text-white/70">
              Open Bazaar SI FEST 2026 dibagi menjadi dua kategori pendaftar. Silakan pilih kategori di bawah ini untuk melihat detail lengkap pendaftaran, biaya, dan persyaratannya.
            </p>
          </div>
        </Container>

        <Container className="relative z-10">
          <EventGrid events={bazaarEvents} />
        </Container>
      </section>
