...
import { MediaPartnerModal } from "@/components/admin/MediaPartnerModal";

export default function MediaPartnersAdmin() {
  ...
  const handleDelete = async (id: string, logoUrl: string) => {
    if (confirm("Apakah Anda yakin ingin menghapus media partner ini?")) {
      const supabase = createClient();
      try {
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={3} className="px-6 py-12 text-center text-slate-400">
                    {searchQuery ? "Tidak ada media partner yang cocok dengan pencarian." : "Belum ada data media partner/partner."}
                  </td>
                </tr>
              ) : (
